...
;this The frame measures 150 pixels wide by 100 pixels height and features a caption that reads "Underground App."
...
(define ShowRoute (new button%
                       ...
                       [callback (lambda (o e)
                                   ...
                                   (println(string-join reachable-2 "Your Route: ----> ") ) (println reachable-2)
...
( define panel ( new horizontal-panel%
                     [parent myframe]))
(define GUI-1 (new message% [label " Your Route:  "][min-width 250]                           
                            [min-height 100]  [vert-margin 10]	 
                            [horiz-margin 10] [parent panel]))
(define GUI-2 (new message% [label ""]
                            [parent panel] [min-width 50][min-height 100]))
...
(define append-lists (λ (x y)
                       (append y (list x))))
; This function add-lists returns the value of (append y (list x)) and accepts the two variable x and y.
...
;As a result, the definition (define append-lists (x y) (append y (list x))) generates a new function called append-lists that accepts the parameters x and y and returns a new list that is the result of concatenating y with a singleton list that contains x.
